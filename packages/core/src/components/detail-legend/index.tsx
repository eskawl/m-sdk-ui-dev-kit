...
const PercentChangeIndicator = ({ value }: PercentChangeIndicatorProps) => {
  ...
  return (
    <span
      className={cn(
        'mining-sdk-detail-legend__percent-change',
        isPositive
          ? 'mining-sdk-detail-legend__percent-change--positive'
          : 'mining-sdk-detail-legend__percent-change--negative',
      )}
    >
...
const DefaultColorBox = ({ color }: DefaultColorBoxProps) => (
  <div
    className="mining-sdk-detail-legend__color-box"
    style={{
...
export const DetailLegend = ({ items, onToggle, className }: DetailLegendProps) => {
  ...
  return (
    <div className={cn('mining-sdk-detail-legend', className)}>
      {items.map((item: DetailLegendItem, index: number) => (
        <button
          key={`${item.label}-${index}`}
          type="button"
          className={cn(
            'mining-sdk-detail-legend__item',
            item.hidden && 'mining-sdk-detail-legend__item--hidden',
          )}
          onClick={() => onToggle?.(item.label, index)}
        >
          <div className="mining-sdk-detail-legend__icon" style={{ color: item.color }}>
            {item.icon ?? <DefaultColorBox color={item.color} />}
          </div>
          <div className="mining-sdk-detail-legend__content">
            <div className="mining-sdk-detail-legend__label">{item.label}</div>
            {item.currentValue && (
              <div className="mining-sdk-detail-legend__value-row">
                <span className="mining-sdk-detail-legend__value">
                  {typeof item.currentValue.value === 'number'
                    ? formatNumber(item.currentValue.value)
                    : item.currentValue.value}
                </span>
                {item.currentValue.unit && (
                  <span className="mining-sdk-detail-legend__unit">{item.currentValue.unit}</span>
                )}
