...
const STATUS_CLASS_MAP: Record<AlertSeverity, string> = {
  [ALERT_SEVERITY_TYPES.CRITICAL]: 'mining-sdk-logs-card__status-icon--critical',
  [ALERT_SEVERITY_TYPES.HIGH]: 'mining-sdk-logs-card__status-icon--high',
  [ALERT_SEVERITY_TYPES.MEDIUM]: 'mining-sdk-logs-card__status-icon--medium',
}
...
const LogDot = ({ type, status }: LogDotProps): JSX.Element | null => {
  const renderMap: Record<string, JSX.Element | null> = {
    [LOG_TYPES.INCIDENTS]: (
      <div className={cn('mining-sdk-logs-card__status-icon', getStatusModifier(status))} />
    ),
